  ...
  try {
    ...
    if (alreadyHasTag) {
      return errorResponse(
        res,
        `Recipe already has tag with id: ${data.tag_id}`
      );
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!recipe) {
        return errorResponse(
          res,
          `Recipe with id: ${recipeId} does not belong to user or not found`
        );
      }
      ...
      if (!result) {
        return errorResponse(
          res,
          `Error in deleting tag with id: ${id} from recipe with id: ${recipeId}`
        );
      }
      ...
      errorResponse(
        res,
        `Error in deleting tag from recipe: ${err.message}`,
        err
      );
    }
  }
